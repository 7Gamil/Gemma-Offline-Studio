...
import SpeakerOnIcon from "../icons/speaker-on.svg";
...
import GemmaSmallIcon from "../icons/Gemma_small.svg";
...
import { LocalAPIContext } from "../context";
import { ChatImage, ChatAudio } from "../typing";
...
function ChatAction(props: {
  ...
  useEffect(() => {
    updateWidth();
  }, [updateWidth, props.text, props.icon]);
...
export function ChatActions(props: {
  ...
  setAttachAudios: (callback: (prev: ChatAudio[]) => ChatAudio[]) => void;
          ...
          mediaRecorder.ondataavailable = (e) => {
            ...
            reader.onloadend = () => {
              ...
              const audio: ChatAudio = {
                url: base64Audio,
                name: "voice_input.webm",
              };
              props.setAttachAudios((prev: ChatAudio[]) => [...prev, audio]);
            };
          };
        })
  ...
  async function saveAudioToFile(audioBlob: Blob) {
    // const a = document.createElement("a");
    // a.href = URL.createObjectURL(audioBlob);
    // a.download = `temp_mic_input_${Date.now()}.webm`;
    // document.body.appendChild(a);
    // a.click();
    // document.body.removeChild(a);
    sendAudioToModel(audioBlob, props.llm);
  }

  async function sendAudioToModel(audioBlob: Blob, llm: any) {
    const reader = new FileReader();
    reader.readAsDataURL(audioBlob);
    reader.onloadend = () => {
      const base64Audio = reader.result as string;
      ...
      chatStore.onUserInput("", llm, [], [audio]);
    };
  }
      ...
      <ChatAction
        onClick={async () => {
          ...
          try {
            ...
            console.error("Failed to force load model:", e);
          }
          ...
          // 3. play the message using tts
          props.llm.tts(message);
        }}
        text={"Coding"}
        icon={<CodingIcon />}
      />
...
export function DeleteButton(props: { onClick: () => void }) {
  return (
    <div className={styles["delete-image"]} onClick={props.onClick}>
      <DeleteIcon />
    </div>
  );
}
...
function _Chat() {
  ...
  const [attachAudios, setAttachAudios] = useState<ChatAudio[]>([]);
  ...
  const localApi = useContext(LocalAPIContext)!;

  const llm = localApi;

  const models = config.models;

  // For playing BOT_HELLO on new chat
  useEffect(() => {
    const lastMessage = session.messages.at(-1);
    if (
      session.messages.length === 1 &&
      lastMessage?.content === BOT_HELLO.content &&
      isSpeakerOn
    ) {
      localApi.tts(getMessageTextContent(lastMessage));
  ...
  useEffect(() => {
    ...
    if (
      lastMessage &&
      lastMessage.id &&
      lastMessage.id !== lastPlayedMessageId &&
  ...
  }, [session.messages, isSpeakerOn, localApi, lastPlayedMessageId]);
  ...
  const onSubmit = (userInput: string) => {
    ...
    chatStore.onUserInput(userInput, llm, attachImages, attachAudios);
    setAttachImages([]);
    setAttachAudios([]);
  ...
  const onResend = (message: ChatMessage) => {
    ...
    const audios = getMessageAudios(userMessage);
    chatStore.onUserInput(textContent, llm, images, audios);
    inputRef.current?.focus();
  };
  ...
  async function uploadFile() {
    ...
    fileInput.type = "file";
    fileInput.accept =
      "image/png, image/jpeg, image/webp, image/heic, image/heif, audio/*, .txt, .pdf";
    ...
    fileInput.onchange = (event: any) => {
      ...
      let newImages = [...attachImages];
      let newAudios = [...attachAudios];
      let newText = "";
      ...
      Promise.all(filePromises).then(() => {
        if (newImages.length > 3) {
          newImages = newImages.slice(newImages.length - 3);
        }
        setAttachImages(newImages);

        if (newAudios.length > 3) {
          newAudios = newAudios.slice(newAudios.length - 3);
        }
        setAttachAudios(newAudios);
    ...
    <div className={styles.chat} key={session.id}>
              ...
              >
                <div className={styles["chat-message-container"]}>
                  ...
                  <div className={styles["chat-message-item"]}>
                    ...
                    {getMessageAudios(message).length > 0 && (
                      <div className={styles["chat-message-item-audios"]}>
                        {getMessageAudios(message).map(
                          (audio: ChatAudio, index: number) => {
                            return (
                              <div
                                key={index}
      ...
      <div className={styles["chat-input-panel"]}>
        ...
        <ChatActions
          ...
          setAttachAudios={setAttachAudios}
        ...
        <label
          className={`${styles["chat-input-panel-inner"]} ${
            attachImages.length != 0 || attachAudios.length != 0
        ...
        >
          ...
          {attachAudios.length != 0 && (
            <div className={styles["attach-audios"]}>
              {attachAudios.map((audio: ChatAudio, index: number) => {
                return (
                  <div key={index} className={styles["attach-audio"]}>
                    <p className={styles["attach-audio-name"]}>{audio.name}</p>
                    <DeleteButton
                      onClick={() => {
                        setAttachAudios(
                          attachAudios.filter((_, i) => i !== index),
                        );
                      }}
                    />
                  </div>
