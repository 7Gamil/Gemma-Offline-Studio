...
const useModels = (
  ...
  useEffect(() => {
    if (config.modelClientType == ModelClient.LOCAL_API) {
      if (localApi) {
        localApi.models().then((models) => {
          config.setModels(models);
        });
      }
    }
  }, [config.modelClientType, localApi]);
};

const useLocalApi = () => {
  const [localApi, setLocalApi] = useState<LocalApi | undefined>(undefined);

  useEffect(() => {
    setLocalApi(new LocalApi());
  }, []);
...
export function Home() {
  ...
  return (
    <ErrorBoundary>
      <Router>
        <LocalAPIContext.Provider value={localApi}>
          <Screen />
